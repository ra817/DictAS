...
#layer to refine & retain features
class MLP(nn.Module):
...
#two residual connections(local happening inside the attention block internally)
#one outside to preserve global semantics generated by the backbone
...
class MyDictionary(nn.Module):
    def __init__(self, config, args):
        ...
        #for each generator we are attaching(q,k,v) so that it can retrieve/address more details about patches
        self.Query_Generator = Attn_Block(dim = config["vision_cfg"]["width"])   #1024
        self.Key_Generator = Attn_Block(dim = config["vision_cfg"]["width"])     #1024
    ...
    #total layers count(during random intialization of weights)
    #linear layer(3 for each attention block)
    #1 block=one generator(query, key, value) total: 18 layers
    #noramlization layers total 6(2 for each generator)
    #3 global fusion layer(lineary)
    #total layers count(18 + 3 + 6 = 27) 18:linear, 6:normalization layer
